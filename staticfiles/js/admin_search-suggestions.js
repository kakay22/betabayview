...
function saveSearchHistory(searchTerm, searchUrl) {
    ...
    const existingItem = history.find(item => item.term === searchTerm);
    ...
    if (history.length > 0) {
        history.forEach(item => {
            ...
            historyItem.innerHTML = `
                <span>${item.term}</span>
                <a class="bi bi-x delete-history-item px-2 p-1 hover:bg-gray-300 rounded-md"></i>
            `;
...
document.getElementById('search-input').addEventListener('input', function () {
    ...
    console.log(searchSuggestionsElement.dataset.pendingAccountsUrl); // Should output the correct URL
    ...
    console.log(searchSuggestionsElement.dataset.pendingAccountsUrl);  // This will help you see the actual value
